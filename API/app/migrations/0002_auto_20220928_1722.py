...
def load(apps, schema_editor):
  ...
  for name, symbol, value in reader2:
    try:
      with transaction.atomic():
        record = Chemical(name=name.strip(), symbol=symbol, value=value)
        record.save()
    except IntegrityError:
      record.delete()
  ...
  for name, kind, spin, charge, mass in reader4:
    try:
      with transaction.atomic():
        record = Particle(name=name.strip(), kind=kind, spin=spin, charge=charge, mass=mass)
        record.save()
    except IntegrityError:
      record.delete()
